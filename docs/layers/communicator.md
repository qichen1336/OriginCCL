...
- **主动边只需要一次 `Connect`，不做重试**（TCP 与 RDMA 一致）：bootstrap 是全体参与的同步交换，而所有 listener 都在 bootstrap **之前**绑好，所以任一 rank 从 bootstrap 返回时其余 rank 必然已在 listen，主动端不可能撞上「对端还没监听」。据此不加重试循环（历史上曾有的 30 次重试是「bootstrap 之后才 listen」时代的产物，那个竞态已不存在），`transport->Connect()` 失败即初始化失败。RDMA 自身由 CM 的 `retry_count`/`rnr_retry_count` 承担超时与重试，语义与 TCP 一致。
